...
byte header [4] = {0x4C, 0x41, 0x52, 0x50};
int healAmount = 0;
int HP = 100;
...
void loop()
{
  ...
  if(ReadDataFromBlock(blockNum, readBlockData)) {
    ...
    if(blockData[4]==0x01) {//check if the item is a healing item, apply heal if it is
      if(blockData[5]>0x00) {
        healAmount = (int)blockData[5];
        
        if(wipeItem(blockData)) { //if healing item sucessfully emptied, heal for the correct amount
          heal(healAmount);
...
bool ReadDataFromBlock(int blockNum, byte readBlockData[]) 
{
  ...
  if (status != MFRC522::STATUS_OK)
  {
    ...
    dump_byte_array(readBlockData, 16); Serial.println();
    Serial.println();
